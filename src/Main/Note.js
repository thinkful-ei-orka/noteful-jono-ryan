...
class Note extends React.Component {
  ...
  render() {
    ...
    let filteredNotesByNote = notes.filter(
      ...
      <section>
        {notesList.map((note) => (
          <div className="noteItem" key={note.id}>
            <NavLink className="navlink note-name" to={'/note/' + note.id}>
              {note.name}
            </NavLink>
            <p className="date">{`Date modified on ${moment(
              note.modified
        ...
        <button className="Add-note" onClick={renderAddNote}>
          Add Note
        </button>
      </section>
    );
